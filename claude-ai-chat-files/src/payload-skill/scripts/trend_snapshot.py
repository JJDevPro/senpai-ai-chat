...
MODI:
  ...
  --local:           ⛔ KEIN Drive — readiness-history.csv von der LOKALEN Disk
                     (--history) lesen, Snapshot mit identischer Rollup+Render-
                     Logik bauen und nach --out-file schreiben. pull_drive wird
                     dabei NIE importiert (läuft ohne google-Libs).
...
CLI:
  ...
  python3 trend_snapshot.py --local --history ./data/readiness-history.csv \
      --out-file ./data/trend_snapshot.md [--as-of YYYY-MM-DD] [--weeks 8] [--months 12]
"""
...
# --------------------------------------------------------------------------- #
# Local-Mode (⛔ KEIN Drive — pull_drive wird hier NIE importiert)
# --------------------------------------------------------------------------- #
def run_snapshot_local(history_path, out_file, as_of=None, weeks=8, months=12):
    """Local-Pendant zu run_snapshot: liest readiness-history.csv vom LOKALEN
    Pfad, fährt exakt denselben Rollup+Render-Pfad wie der Drive-Modus
    (read_history → rollup_weekly/rollup_monthly → render_snapshot) und
    schreibt das Snapshot-Markdown nach out_file (Parent-Ordner wird angelegt).

    Gibt (out_path, summary_dict) zurück; summary = kompakte Kennzahlen für den
    CLI-Print (Tageszeilen/Wochen-/Monats-Buckets). Fehlt die History-CSV,
    SystemExit(2) mit klarer Fehlermeldung (Pendant zur Vor-Seed-Anweisung).
    """
    hist_file = Path(history_path)
    if not hist_file.is_file():
        rh._eprint(f"ERROR: --history {str(hist_file)!r} nicht gefunden "
                   f"(Local-Mode liest NUR die lokale Disk, kein Drive).")
        raise SystemExit(2)
    hist = hist_file.read_text(encoding="utf-8")
    rows = rh.read_history(hist)
    weekly = rollup_weekly(rows, weeks, as_of=as_of)
    monthly = rollup_monthly(rows, months, as_of=as_of)
    md = render_snapshot(weekly, monthly, as_of=as_of)
    out_path = Path(out_file)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(md, encoding="utf-8")
    return out_path, {"days": len(rows), "weeks": len(weekly), "months": len(monthly)}
...
def main(argv=None):
    ...
    ap.add_argument("--local", action="store_true",
                    help="Local-Mode: ⛔ KEIN Drive — --history lesen, Snapshot nach --out-file schreiben.")
    ap.add_argument("--history", help="lokaler Pfad zur readiness-history.csv (nur mit --local).")
    ap.add_argument("--out-file", dest="out_file",
                    help="lokaler Zielpfad für das Snapshot-Markdown (nur mit --local).")
    args = ap.parse_args(argv)

    if args.local:
        # ⛔ Ab hier KEIN Drive-Zugriff — pull_drive bleibt unimportiert (google-Lib-frei).
        if not args.history or not args.out_file:
            print("--local braucht --history und --out-file", file=sys.stderr)
            return 1
        out_path, s = run_snapshot_local(args.history, args.out_file, as_of=args.as_of,
                                         weeks=args.weeks, months=args.months)
        print(out_path)
        print(f"Snapshot (lokal): {s['days']} Tageszeilen → {s['weeks']} Wochen-Buckets · "
              f"{s['months']} Monats-Buckets" + (f" · Stand {args.as_of}" if args.as_of else ""))
        return 0
