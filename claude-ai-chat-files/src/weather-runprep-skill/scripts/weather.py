...
Offline-Modi (KEIN Netz — für Tests/Replays):
  --from-json <path>  Bright-Sky-Roh-Response aus Datei statt HTTP laden,
                      identisch durch reduce() geschickt wie der Fetch-Pfad.
  --print-url         nur die Fetch-URL für die Args drucken und beenden.
"""
...
def build_url(lat: float, lon: float, date: str, tz: str = DEFAULT_TZ) -> str:
    """Fetch-URL deterministisch bauen (BASE + urlencode) — geteilt von fetch()
    und dem Offline-Flag --print-url, damit beide Pfade NIE auseinanderdriften."""
...
def main(argv=None):
    ...
    p.add_argument("--from-json", dest="from_json", metavar="PATH",
                   help="OFFLINE: Bright-Sky-Roh-Response aus dieser Datei statt HTTP laden (kein Netz)")
    p.add_argument("--print-url", dest="print_url", action="store_true",
                   help="OFFLINE: nur die Fetch-URL für die Args drucken und beenden (kein Netz)")
    args = p.parse_args(argv)
    if args.print_url:
        # Offline-Kurzschluss: URL zeigen, nichts ziehen — z. B. für Debugging/Curl.
        print(build_url(args.lat, args.lon, args.date, args.tz))
        return 0
    if args.from_json:
        # Offline-Replay: Roh-Response aus Datei — identischer reduce()-Pfad wie fetch().
