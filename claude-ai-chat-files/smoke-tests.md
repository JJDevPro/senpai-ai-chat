...
> Hole per Web-Fetch diese URL und zeig mir NUR die erste Stunde des JSON: die URL aus `assets/brightsky_url.txt` des weather-Bundles (`{date}` = heute, YYYY-MM-DD) — danach hole den Wetterochs-RSS (wetterochs.de) und fasse die aktuelle Ausgabe in einem Satz zusammen.
