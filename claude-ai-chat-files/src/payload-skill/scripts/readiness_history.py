...
LOCAL-MODE (--csv-path <file>): KEIN Drive-Zugriff, KEIN pull_drive-Import — die CSV
  wird direkt vom lokalen Pfad gelesen (fehlend → MIT Header-Zeile neu angelegt; die
  Vor-Seed-Regel oben ist eine Drive-Quota-Restriktion und gilt lokal nicht), die
  Tages-Zeile identisch zum Drive-Modus via append_row angehängt (gleiches Datum →
  idempotenter No-op, nie ein Duplikat) und zurückgeschrieben. Statt der Drive-File-ID
  druckt der Local-Mode die kompakte JSON-Zusammenfassung
  {"date":…,"appended":…,"rows":…,"csv":<pfad>}. --csv/--folder/--out/--sa-file
  werden im Local-Mode ignoriert. Läuft in Umgebungen OHNE google-Libs.
...
# --------------------------------------------------------------------------- #
# Local-Mode (--csv-path): KEIN Drive — reine Datei-I/O um dieselbe append_row-Logik
# --------------------------------------------------------------------------- #
def run_history_local(row_dict, csv_path):
    """Local-Mode-Gegenstück zu run_history: CSV vom LOKALEN Pfad lesen → Zeile via
    append_row anhängen → zurückschreiben. Gibt die kompakte Zusammenfassung als
    dict zurück ({"date","appended","rows","csv"}); main() druckt sie als JSON.

    ⛔ KEIN Drive-Zugriff, KEIN pull_drive-Import — der Import in run_history ist
      bewusst lazy, dieser Pfad fasst ihn nie an und läuft daher auch in einer
      Umgebung OHNE google-Libs.
    Fehlt die Datei, wird sie MIT Header-Zeile neu angelegt — die Vor-Seed-Regel
      (run_history) ist eine Drive-Quota-Restriktion des Service-Accounts und gilt
      für eine lokale Datei nicht.
    Rechenpfad = identisch zum Drive-Modus: dieselbe append_row-Idempotenz
      (gleiches Datum → No-op, nie ein Duplikat, keine stille Überschreibung).
    """
    p = Path(csv_path)
    if p.exists():
    ...
    return {
        ...
        "appended": updated != current,   # False = Datum existierte schon (No-op)
        "rows": len(read_history(updated)),
        "csv": str(p),
    }
...
def main(argv=None):
    ...
    ap.add_argument("--csv-path", dest="csv_path",
                    help="Local-Mode: CSV unter diesem lokalen Pfad lesen/schreiben (fehlend → "
                         "mit Header anlegen) — KEIN Drive-Zugriff; --csv/--folder/--out/--sa-file "
                         "werden ignoriert. Druckt die kompakte JSON-Zusammenfassung statt der Drive-File-ID.")
    ...
    # Local-Mode: kein Drive, keine google-Libs — direkt gegen den lokalen Pfad.
    if args.csv_path:
        summary = run_history_local(row_dict=row, csv_path=args.csv_path)
